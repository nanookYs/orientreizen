...
function ic_add_product_category_image( $term_id ) {
	if ( is_ic_category_image_enabled() ) {
		...
		echo wp_get_attachment_image( $term_img, apply_filters( 'product_cat_image_size', 'product-category-page-image' ), false, array( 'class' => 'product-category-image' ) );
	}
}
...
function ic_get_template_file( $file_path ) {
	...
	} else if ( file_exists( AL_BASE_PATH . '/templates/template-parts/' . $file_path ) ) {
 ...
 * @return type
 */
function ic_show_template_file( $file_path ) {
	$path = ic_get_template_file( $file_path );
	if ( $path ) {
		include $path;
	}
	return;
